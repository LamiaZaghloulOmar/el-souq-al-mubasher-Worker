...
class AuthRepo {
  ...
  Future<Response> login(String phone, String password) async {
    return await apiClient.postData(
        AppConstants.LOGIN_URI, {"email": phone, "password": password});
  }

  Future<Response> getProfileInfo() async {
    return await apiClient
        .getData(AppConstants.PROFILE_URI + '/' + getUserToken());
  }
  ...
  Future<bool> saveUserToken(String token, bool isZone,
      {String zoneTopic}) async {
    apiClient.token = token;
    apiClient.updateHeader(
        token, sharedPreferences.getString(AppConstants.LANGUAGE_CODE));
    if (isZone) {
      CacheHelper.saveBoolData(key: "isDeliver", value: true);
      sharedPreferences.setString(AppConstants.ZONE_TOPIC, zoneTopic);
    } else {
      CacheHelper.saveBoolData(key: "isDeliver", value: false);
      print("SaveId");
    }
    print("IsDeliver = ${CacheHelper.getData(key: "isDeliver")}");

    return await CacheHelper.saveStringData(
        key: AppConstants.TOKEN, value: token);
  }
